...
def compute_reward(actions, feature, key_feature, probs, key_probs, r_output, key_r_output):
    ...
    for i in range(n_originalVideo):
        sum_originalVideo = sum_originalVideo + probs[i] * feature[i]
    Cv = sum_originalVideo / n_originalVideo

    #计算Cs
    sum_summaryVideo = 0.0
    for i in range(n_summaryVideo):
        sum_summaryVideo = sum_summaryVideo + key_probs[i] * key_feature[i]
